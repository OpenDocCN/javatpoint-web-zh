...
React引导包是在React应用程序中添加引导的最常见方式。社区构建了许多引导包，旨在将引导组件重建为React组件。两个最受欢迎的引导包是:

1.  **react-bootstrap:** 它是 bootstrap 组件作为 react 组件的完全重新实现。它不需要像 bootstrap.js 或 jQuery 这样的依赖关系。如果安装了React设置和React引导，我们就拥有了所需的一切。
...
让我们使用**创建-React-应用程序**命令创建一个新的React应用程序，如下所示。
...
创建 React 应用程序后，安装 Bootstrap 的最佳方式是通过 npm 包。要安装引导程序，请导航到React应用程序文件夹，并运行以下命令。
...
让我们使用如下创建-React-应用命令创建一个新的React应用。
...
接下来，通过 npm 包安装**React堆捕集器**。要安装 reactstrap，请导航到 React 应用程序文件夹，并运行以下命令。
