...
Angular引导<mat-button>用于创建带有内容样式和动画的按钮。有Angular的内容按钮是原生的<button>或通过样式和墨水波纹增强的元素。</button></mat-button>
...
Angular内容使用原始的<button>和<diagram>元素，以确保默认情况下的无障碍体验。</diagram></button><button>元素用于作用于当前页面的任何交互。</button>
