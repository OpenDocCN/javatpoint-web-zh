...
钩子是 React 16.8 版本中引入的新功能。它允许您在不编写类的情况下使用状态和其他 React 特性。钩子是从功能组件“钩入”React状态和生命周期特征的功能。它在类内不起作用。

钩子是向后兼容的，这意味着它不包含任何中断的更改。此外，它不会取代你的React概念的知识。
...
不要在循环、条件或嵌套函数中调用 Hooks。钩子应该总是用在React函数的顶层。该规则确保每次组件呈现时以相同的顺序调用钩子。
...
1.  节点版本 6 或更高版本
2.  NPM 版本 5.2 或以上
3.  用于运行React应用程序的创建React应用程序工具

## 反作用钩安装

要使用React挂钩，我们需要运行以下命令:
...
上面的命令将安装最新的反作用和反作用 DOM 阿尔法版本，支持反作用钩子。确保**包. json** 文件列出了如下所示的React和React-DOM 依赖关系。
...
#### 注意:自定义钩子是一种惯例，它自然地遵循钩子的设计，而不是React特性。
