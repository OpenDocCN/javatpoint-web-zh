...
惰性加载是一种有Angular的技术，允许您在特定路线被激活时加载 JavaScript 组件。它通过将应用程序分成许多包来提高应用程序加载的速度。当用户通过应用程序导航时，会根据需要加载包。
...
在惰性加载Angular模块中，在您的**approxing module**根配置中使用加载子模块，如下所示。
