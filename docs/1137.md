...
Angular CLI 是一个命令行界面工具，用于初始化、开发、搭建和维护 Angular 应用程序。您可以直接在命令提示符下使用这些命令，也可以通过交互式用户界面(如Angular控制台)间接使用这些命令。
...
| e2e | e | 它构建并服务于一个 Angular 应用程序，然后使用量Angular器进行端到端的测试。 |
...
它用于构建和服务一个 Angular 应用程序，然后使用量Angular器进行端到端测试。
...
**- baseUrl=baseUrl:** 指定量Angular器要连接的基本 Url。
...
**-elementExplorer = true | false:**启动量Angular器的 Element Explorer 进行调试。
...
**-量Angular器配置文件=量Angular器配置文件:**指定量Angular器配置文件的名称。

**-规格:**它覆盖了量Angular器配置中的规格。

**-套件=套件:**它覆盖了量Angular器配置中的套件。
