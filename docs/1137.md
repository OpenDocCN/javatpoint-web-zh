...
> 原文:[https://www.javatpoint.com/angular-7-all-cli-commands](https://www.javatpoint.com/angular-7-all-cli-commands)

Angular CLI 是一个命令行界面工具，用于初始化、开发、搭建和维护 Angular 应用程序。您可以直接在命令提示符下使用这些命令，也可以通过交互式用户界面(如角度控制台)间接使用这些命令。
...
| e2e | e | 它构建并服务于一个 Angular 应用程序，然后使用量角器进行端到端的测试。 |
...
![All Angular CLI commands](../Images/f204cd4bb6dccf5871e6c42b805b413c.png)
...
它用于构建和服务一个 Angular 应用程序，然后使用量角器进行端到端测试。
...
**- baseUrl=baseUrl:** 指定量角器要连接的基本 Url。
...
**-elementExplorer = true | false:**启动量角器的 Element Explorer 进行调试。
