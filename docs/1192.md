...
| 挂钩方法 | 目的 | 时间 |
| ngOnchange() | 当Angular设置或重置数据绑定输入属性时做出反应。方法接收当前和以前属性值的简单更改对象。
...
| ngAfterContentInit() | 在Angular投影后，外部内容进入组件视图或具有指令的反应视图。
参见本文档中响应内容变化的细节和示例。 | 在第一个 ngDoCheck()之后调用一次。 |
| ngOnDestroy() | 在销毁Angular指令或组件之前进行清理。为了避免内存泄漏，取消订阅可观察的对象并分离事件处理程序。请参阅本文档中实例销毁时的清理中的详细信息。 | 在销毁 Angular 指令或组件之前立即调用。 |
