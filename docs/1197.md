...
应用程序中的每个 Angular 模板都是一大块 HTML，包含在浏览器显示的页面中。有Angular的 HTML 模板在浏览器中呈现视图或用户界面，就像普通的 HTML 一样，但功能更多。
...
Angular 支持双向数据绑定，这是一种用于协调模板部分和组件部分的机制。请参阅将绑定标记添加到模板 HTML，了解如何在Angular中添加两侧。
...
Angular形管道允许您在模板中声明显示值的变化。@Pipe 带有装饰器的类定义了一个函数，该函数将输入值转换为显示在视图中的输出值。
...
Angular模板是动态的。当 Angular 渲染它们时，它会根据指令给出的指令改变 DOM。指令是一个带有@Directive()装饰器的类。
